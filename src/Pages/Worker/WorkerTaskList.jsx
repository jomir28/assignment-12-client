...
const WorkerTaskList = () => {
    ...
    const { data: allTask, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const { data } = await axiosSecure(`/all-task?page=${currentPage}&size=${itemsPerPage}`)
            return data;
        }
    })
    ...
    console.log(numberOfPages, 'hello world');
    ...
    return (
        <div>
            ...
            <div className="flex justify-center my-5 gap-5">
                <button onClick={handlePrevPage}><GrLinkPrevious></GrLinkPrevious></button>
                {
                    pages.map(page => <button onClick={()=>setCurrentPage(page)}  className={`btn btn-primary ${currentPage === page ? 'bg-orange-400 text-black' : undefined}`} key={page}>{page}</button>)
                }
                <button onClick={handleNextPage}><GrLinkNext></GrLinkNext></button>
