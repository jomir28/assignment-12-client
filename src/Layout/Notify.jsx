...
const Notify = ({ notify, refetch }) => {
    ...
    const handleStatusChanged = async(id) => {
        const data = await axiosSecure.patch(`/notification/${id}`,{status:'read'})
        console.log(data);
        refetch()
    ...
    return (
        <div className="bg-slate-100 text-sm mb-2 px-2 py-1 rounded-2xl">
            ...
            <div className="flex justify-between">
                ...
                <p>Time: {timeString}</p>
                {notify.status==='unread'&&<button onClick={()=>handleStatusChanged(notify._id)} className="underline text-green-500">Read</button>}
            </div>
        </div>
    );
};
